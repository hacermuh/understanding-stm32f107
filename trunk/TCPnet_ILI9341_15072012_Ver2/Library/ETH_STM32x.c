...
//extern GLCD_displayStringLn(unsigned int ln, unsigned char *s);
/*----------------------------------------------------------------------------
...
void init_ethernet (void) {
  ...
  if (((id1 << 16) | (id2 & 0xFFF0)) == DP83848C_ID) {
     GLCD_displayStringLn(Line3, "DP83848C Detected");  
    ...
    for (tout = 0; tout < 0x10000; tout++) {
      ...
      if (regv & 0x0020) {
	    ...
	    GLCD_displayStringLn(Line4, "Run Autonegotiation");
        break;
      }
    }
#endif
  ...
  for (tout = 0; tout < 0x10000; tout++) {
    ...
    if (regv & 0x0001) {
      /* Link is on. */
	  GLCD_displayStringLn(Line5, "Link is on");
      break;
    }
  }
